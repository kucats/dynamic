...
- 3D trombone player (`public/reader/trombone3d.js`, see `docs/trombone-3d.md`): opt-in per viewer, trombone parts only, and it loads the vendored Three.js only when switched on. It shows the reader's `pos` values; it never infers positions from pitch.
