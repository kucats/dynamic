...
const [input, part, movement, referenceInput, windowArgument = '32', control = 'none'] = process.argv.slice(2);
...
const matcher = new PhraseMatcher(compileScore(reader, movement, profile));
...
if (referenceInput) {
  ...
  if (control === 'shuffle') {
    ...
    const shuffled = query.map((f) => f.chroma); let seed = 94731;
...
for (let i = 0; i < floats.length; i += 4098) {
  const time = floats[i], rms = floats[i + 1], db = floats.subarray(i + 2, i + 4098);
  const start = performance.now();
  const result = matcher.push(time, rms >= .008 ? spectralChroma(db, 48000, 8192) : null);
  timings.push(performance.now() - start);
...
timings.sort((a, b) => a - b);
console.log(JSON.stringify({ algorithm: 'local-ensemble-context-dtw-v2', part, movement, frames, counts, rewinds,
