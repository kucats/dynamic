...
def build(part_dir: Path, pdf: Path, work: Path) -> dict:
    ...
    for page in pages:
        ...
        for n in data["notes"]:
            ...
            elif instr == "clarinet":      # in A / Bb; 'f' = how a Bb clarinet reads it
            ...
            elif instr == "viola":         # non-transposing, alto clef, sounds as written
                w_ = label(L, a, wo)
                rec.update(key="C", w=w_, f=w_, snd=w_)
            elif instr == "cello":          # non-transposing strings; solfège labels
                w_ = label(L, a, wo)
                rec.update(key="C", w=w_, f=w_, snd=w_)
            elif instr == "trumpet":       # crook transposes up (in Re = +M2); f column = sounding
            ...
            elif instr == "english-horn":   # in F: sounds a fifth below written
                dl, ds = HORN_KEYS["F"]
                s_ = transpose(L, a, wo, dl, ds)
                rec.update(key="F", w=label(L, a, wo), f=label(L, a, wo), snd=label(*s_))
            elif instr != "horn":          # non-transposing instruments (violin, etc.)
                w_ = label(L, a, wo)
                rec.update(key="C", w=w_, f=w_, snd=w_)
            else:
