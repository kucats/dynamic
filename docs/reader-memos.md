...
- **ログインしていないとき**：リーダーに「メモ：未ログイン」と表示します。「メモ」を押すか、譜面の空白をダブルタップ／ダブルクリックすると、ログイン案内が開きます。Googleログインの後に「ログインしました」とメールアドレスを表示する確認画面を出し、「続ける」を押すと元のパートへ戻ります。ログイン中はリーダーにメールアドレスが表示されます。
