...
ログイン中は小節メニューの先頭に **◎ ○ △ ×** を横並びで表示します。押すと入力画面を開かず、その小節に日時付きの練習メモを1件追加します。◎・○は「できた」、△・×は「課題」で、本文は押した記号です。同じ小節に付け直しても過去の印は残り、一覧で日付を確認できます。青い選択状態はその小節の最新の印です。ログインしていない場合やメモ機能が無効なパートでは印を表示しません。保存失敗は画面上部に表示し、成功した印として扱いません。
