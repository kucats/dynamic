...
| DYNAMIC reader JavaScript | `node --check public/reader/app.js && node --check public/reader/memo.js && node --check public/reader/memo-model.mjs && node --check public/reader/metronome.mjs && node --check public/reader/conductor.mjs && node --check public/reader/score-viewer.js && node --check public/reader/trombone3d.js && node --check public/reader/trombone3d-addon.js && node --check public/sw.js` | No syntax errors |
| Reader anchors, metronome beat grid, conductor patterns + 3D trombone kinematics | `node --test tests/reader/*.test.mjs` | Measure anchors, legacy memo fallback, in 1/2/3/4/6 choice and baton ictus timing, slide extensions, constant arm bones, reach and bell landmarks pass |
...
| Tuner pitch detection | `node --check public/reader/tuner-addon.js && node --check public/reader/tuner.mjs && node --test tests/reader/tuner-pitch.test.mjs` | Synthetic tones 58–1047 Hz within 1 cent, weak-fundamental brass spectra not octave-shifted, silence/noise rejected, reader note names |
| Experimental reader following | `node --check public/reader/follow-addon.js && node --check public/reader/follow.mjs && node --check public/reader/follow-worker.mjs && node --check public/reader/follow-alignment.mjs && node --test tests/reader/follow-model.test.mjs && python3 tools/dynamic/build_following.py --check` | Uncertain-note exclusion, ambiguous repeats, sticky manual overrides, constant-BPM prediction, bounded 3-minute causal replay with tempo changes, long-context disambiguation, equal DTW query weights, stale-profile rejection; generated profiles reproducible. See `docs/score-following-reader.md`; these are not ensemble accuracy tests. |
| Nearby-part diagnostic | `node --check tools/dynamic/following_part.mjs && node --check tools/dynamic/following_replay.mjs && node --test tests/reader/follow-part.test.mjs` | Original synthetic weak-fundamental spectra, octave identity, unresolved-note exclusion, wrong/omitted/extra sounds, rest prediction and manual priority. Diagnostic only; no ensemble accuracy or source-separation claim. |
| Part event-sequence diagnostic | `node --check tools/dynamic/following_events.mjs && node --check tools/dynamic/following_controls.mjs && node --test tests/reader/follow-events.test.mjs && python3 -m py_compile tools/dynamic/evaluate_following.py` | Causal event segmentation, ties/repeats, explicit substitution/insertion/deletion, sparse-event normalization, ambiguous motifs, automatic synthetic acquisition, rest recovery, manual priority and frame-preserving block control. Offline only; no real-recording accuracy claim. |
