...
class FuyomiWrapperTests(unittest.TestCase):
    @patch("tools.fuyomi.score_reading.subprocess.run")
    def test_score_wrapper_forwards_argv_without_shell(self, run):
        run.return_value.returncode = 7
        self.assertEqual(score_reading.main(["review", "workspace with spaces"]), 7)
        args, kwargs = run.call_args
        self.assertEqual(args[0], ["vedit", "specialized", "fuyomi", "review", "workspace with spaces"])
